...
const {CONSIDERED_EVENTS,DATA_TEST_ID_KEY} = require('../Enums');
...
function reportForVModel(context, node, firstToken, vModelKey) {
  context.report({
    ...
    fix: (fixer) => {
      return fixer.insertTextAfter(firstToken, ` ${DATA_TEST_ID_KEY}="${vModelKey}"`);
    }
  })
}
...
function reportForEvent(context, node, firstToken, reportFixValue) {
  context.report({
    ...
    fix: (fixer) => {
      return fixer.insertTextAfter(firstToken, ` ${DATA_TEST_ID_KEY}="${reportFixValue}"`);
    }
  })
}
  ...
  create: function (context) {
    ...
    return defineTemplateBodyVisitor(context, {
      VElement(node) {
        ...
        if (isAttributePresent(attributes, DATA_TEST_ID_KEY)) {
          return;
        }
